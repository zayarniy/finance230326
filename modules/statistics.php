...
// Получение данных для статистики (исключаем переводы)
$sql = "SELECT t.*, c.name as category_name, c.color as category_color, 
        ...
        AND t.type != 'transfer'"; // Исключаем переводы из статистики
...
// Общая статистика (только доходы и расходы, без переводов)
$total_income = 0;
$total_expense = 0;
...
// Статистика по категориям (только для выбранного типа)
$categories_stats = [];
foreach ($transactions as $trans) {
...
$tags_list = [];
$stmt = $pdo->prepare("SELECT tags_text FROM transactions WHERE user_id = ? AND tags_text IS NOT NULL AND tags_text != '' AND type != 'transfer'");
$stmt->execute([$user_id]);
...
<head>
    ...
    <style>
        * { -webkit-tap-highlight-color: transparent; }
        ...
        .transaction-item { padding: 12px 0; border-bottom: 1px solid #e9ecef; }
...
<body>
    ...
    <div class="filter-bar">
        <div class="small text-muted mb-2">
            ...
            <?php if ($filter_type != 'all'): ?>
                <span class="badge bg-secondary ms-2">Тип: <?php echo $filter_type == 'income' ? 'Доходы' : 'Расходы'; ?></span>
            <?php endif; ?>
        </div>
    </div>
        ...
        <?php foreach ($categories_stats as $cat): ?>
            <div class="category-item">
                <div class="d-flex justify-content-between small mb-1">
                    <span>
                        <span style="display:inline-block; width:10px; height:10px; background:<?php echo $cat['color']; ?>; border-radius:50%;"></span>
                ...
                <div class="progress">
                    <div class="progress-bar" style="width: <?php echo ($cat['total'] / $max_total) * 100; ?>%; background: <?php echo $cat['color']; ?>"></div>
                </div>
            </div>
        <?php endforeach; ?>
    </div>
                ...
                <?php foreach (array_slice($transactions, 0, 10) as $t): ?>
                    <div class="transaction-item d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <span class="badge" style="background: <?php echo $t['category_color']; ?>"><?php echo htmlspecialchars($t['category_name']); ?></span>
                        </div>
    ...
    <div class="mobile-nav">
        <div class="nav-scroll">
            ...
            <a href="statistics.php" class="nav-item active"><i class="bi bi-graph-up-fill"></i><span>Статистика</span></a>
