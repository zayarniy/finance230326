...
$params = [$user_id];
if ($filter_status == 'active') $sql .= " AND is_active = 1";
if ($filter_status == 'inactive') $sql .= " AND is_active = 0";
...
<head>
    ...
    <style>
        * { -webkit-tap-highlight-color: transparent; }
        ...
        .account-card.inactive { opacity: 0.6; background: #f8f9fa; }
        ...
        .icon-selector { display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; margin-top: 8px; }
        .icon-option {
            text-align: center; padding: 10px; border: 2px solid #e9ecef;
            border-radius: 12px; cursor: pointer; font-size: 24px;
        }
        .icon-option.selected { border-color: #667eea; background: #f0f0ff; }
    </style>
</head>
<body>
    ...
    <div class="stats-grid">
        <div class="stat-card"><div>💰</div><div class="stat-value"><?php echo number_format($total_balance, 0, '.', ' '); ?> ₽</div><div class="small">Общий баланс</div></div>
        <div class="stat-card"><div>🏦</div><div class="stat-value"><?php echo $active_count; ?> / <?php echo count($accounts); ?></div><div class="small">Активных счетов</div></div>
    </div>
    ...
    <div class="account-card <?php echo !$acc['is_active'] ? 'inactive' : ''; ?>" data-id="<?php echo $acc['id']; ?>" data-number="<?php echo htmlspecialchars($acc['account_number']); ?>" data-bank="<?php echo htmlspecialchars($acc['bank_name']); ?>" data-note="<?php echo htmlspecialchars($acc['note'] ?? ''); ?>" data-date="<?php echo $acc['opening_date']; ?>" data-color="<?php echo $acc['color']; ?>" data-icon="<?php echo $acc['icon']; ?>" data-active="<?php echo $acc['is_active']; ?>" data-balance="<?php echo $acc['current_balance']; ?>">
        <div class="d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center gap-3">
                <!--
                <div class="account-icon" style="background: <?php echo $acc['color']; ?>20; color: <?php echo $acc['color']; ?>"><?php echo $acc['icon']; ?></div>-->
                <div><div class="fw-semibold"><?php echo htmlspecialchars($acc['bank_name']); ?></div><div class="small text-muted"><?php echo htmlspecialchars(substr($acc['account_number'], -8)); ?></div></div>
            </div>
            <div class="fw-bold <?php echo $acc['current_balance'] >= 0 ? 'balance-positive' : 'balance-negative'; ?>"><?php echo number_format($acc['current_balance'], 0, '.', ' '); ?> ₽</div>
        </div>
    ...
    <div class="mobile-nav">
        <div class="row g-0">
            ...
            <div class="col-2"><a href="accounts.php" class="nav-item active"><i class="bi bi-bank"></i><span>Счета</span></a>
            </div>
    ...
    <div class="modal fade" id="filterModal" tabindex="-1">
        <div class="modal-dialog"><div class="modal-content">
            <div class="modal-header"><h5>Фильтр</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
    ...
    <div class="modal fade" id="accountModal" tabindex="-1">
        <div class="modal-dialog"><div class="modal-content">
            <div class="modal-header"><h5 id="modalTitle">Добавить счет</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
            <form method="POST">
                <div class="modal-body">
                    <input type="hidden" name="account_id" id="accountId">
                    <input type="text" name="bank_name" id="bankName" class="form-control mb-2" placeholder="Название банка" required>
                    <input type="text" name="account_number" id="accountNumber" class="form-control mb-2" placeholder="Номер счета" required>
                    <input type="number" name="initial_balance" id="initialBalance" class="form-control mb-2" placeholder="Начальный баланс" step="0.01" value="0">
                    <input type="date" name="opening_date" id="openingDate" class="form-control mb-2" value="<?php echo date('Y-m-d'); ?>">
                    <textarea name="note" id="note" class="form-control mb-2" rows="2" placeholder="Примечание"></textarea>
                    <input type="color" name="account_color" id="accountColor" class="form-control mb-2" value="#6c757d">
                ...
                <div class="modal-footer"><button type="submit" name="add_account" class="btn btn-primary w-100 rounded-pill" id="submitBtn">Добавить</button></div>
            </form>
        </div></div>
    </div>
    ...
    <div class="modal fade" id="adjustModal" tabindex="-1">
        <div class="modal-dialog"><div class="modal-content">
            <div class="modal-header"><h5>Корректировка баланса</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
            <form method="POST">
                <div class="modal-body">
                    ...
                    <input type="number" name="new_balance" class="form-control" step="0.01" required placeholder="Новый баланс">
                </div>
                <div class="modal-footer"><button type="submit" name="adjust_balance" class="btn btn-warning w-100 rounded-pill">Скорректировать</button></div>
            </form>
        </div></div>
    </div>
    ...
    <div class="modal fade" id="deleteModal" tabindex="-1">
        <div class="modal-dialog"><div class="modal-content">
            <div class="modal-header"><h5>Удалить счет?</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
            <form method="POST">
                <div class="modal-body">
                    <input type="hidden" name="account_id" id="deleteId">
                    <p>Удалить счет <strong id="deleteName"></strong>?</p>
                </div>
                <div class="modal-footer"><button type="submit" name="delete_account" class="btn btn-danger w-100 rounded-pill">Удалить</button></div>
            </form>
        </div></div>
    </div>
